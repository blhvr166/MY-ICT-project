...
include("config.php");
        ...
        $slot_query = $connection -> query(" SELECT * FROM slot WHERE salon_id = (SELECT salon_id FROM salon WHERE email = '$user_check')");
        ...
        $booking_query = $connection -> query ("SELECT avail, date, slot.emp_id, slot.user_id, employee.fname, slot.customer, slot.c_number
        FROM slot, employee
        WHERE slot.emp_id = employee.emp_id
        AND slot.salon_id = '$salon_id'
...
<div class="container  " style="text-align:center; min-height: 700px">
	...
	<img class="rounded-circle mx-auto"
		...
		<a  href=salon_update.php class="btn btn-outline-danger mt-2" style="float:center" >EDIT </a> 
		<a  href=salon_welcome.php class="btn btn-outline-danger mt-2" style="float:center"> ADD EMPLOYEE</a>
		<a  href=salon_serv.php class="btn btn-outline-danger mt-2" style="float:center"> ADD SERVICES</a>
	</div>
